...
class BaseProxy {
  ...
  async submit(requestType, url, data = null, ...config) {
    ...
    data = requestType === 'get' ? [{ params: data, ...config }] : [data, ...config];
    const response = await Vue.$http[requestType](url + this.getParameterString(), ...data);
    return response;
  }
   ...
   */
  async all() {
    const response = await this.submit('get', `/${this.path}`);
    return response;
  }
   ...
   */
  async find(id) {
    const response = await this.submit('get', `/${this.path}/${id}`);
    return response;
  }
